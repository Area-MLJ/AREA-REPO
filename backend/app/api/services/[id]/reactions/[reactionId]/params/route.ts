...
export async function GET(
  ...
  try {
    const { data: paramsData, error } = await db.from('service_reaction_params')
      .select('id, name, display_name, data_type, required, position, enum_values, default_value, description')
      .eq('service_reaction_id', params.reactionId)
      .order('position')
      .order('name');

    if (error) {
      logger.error('Get reaction params error', { error });
    ...
    logger.error('Get reaction params error', { error: error.message, stack: error.stack });
    return NextResponse.json<ApiResponse>(
      { success: false, error: 'Erreur lors de la récupération des paramètres' },
      { status: 500 }
    );
