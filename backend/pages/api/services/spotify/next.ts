...
import { SpotifyApiError, spotifyApiPost } from '../../../../src/services/spotify/client'
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const path = deviceId ? `/v1/me/player/next?device_id=${encodeURIComponent(deviceId)}` : '/v1/me/player/next'

    await spotifyApiPost(path, accessToken)

    Logger.logResponse(res, 204, null)
    res.status(204).end()
  } catch (error) {
    Logger.logError(error, 'SPOTIFY_NEXT_ERROR')
